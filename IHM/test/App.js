...
const GAMES = [
    { id: 1,  name: 'Tic-Tac-Toe',   icon: '⭕', color: '#ffffff', type: 'ttt'     },
...
function buildGameList() {
    ...
    GAMES.forEach(g => {
        ...
        btn.id = 'gbtn-' + g.id;
        btn.innerHTML = `<span class="gid" style="background:${g.color}22;color:${g.color}">${g.id}</span><span>${g.icon} ${g.name}</span>`;
...
function drawTTT(s) {
    ...
    const board=s.board, sz=Math.min(W,H)*0.78;
...
canvas.addEventListener('click', e => {
    ...
    const W=canvas.width, H=canvas.height;
    const sz=Math.min(W,H)*0.78, ox=(W-sz)/2, oy=(H-sz)/2, cell=sz/3;
...
function drawRush(s) {
    ...
    (s.vehicles??[]).forEach((v,idx) => {
        ...
        ctx.textAlign='center'; ctx.textBaseline='middle';
        ctx.fillText(v.id, rx+rw/2, ry+rh/2);
    });
...
function drawLander(s) {
    ...
    for (let i=0;i<60;i++) {
        const sx=(i*137.5)%W, sy=(i*97.3)%(H*0.85), r=i%4===0?1.2:0.6;
    ...
    const pad=s.landing_pad??{x1:40,x2:60,y:0};
    ...
    for (let i=0;i<=5;i++) {
        const lx=padX1+(padX2-padX1)*i/5;
        ctx.fillStyle=i%2===0?'#00e676':'#fff';
        ctx.beginPath(); ctx.arc(lx,groundY-2,2,0,Math.PI*2); ctx.fill();
    }

    const pos=s.position??{}, vel=s.velocity??{};
    const shipX=toX(pos.x??world.width/2), shipY=toY(pos.altitude??0);
    const tilt=s.tilt??0;
    ctx.save(); ctx.translate(shipX,shipY); ctx.rotate(tilt);
    ctx.fillStyle='#e0e0e0';
    ctx.beginPath(); ctx.moveTo(0,-14); ctx.lineTo(8,10); ctx.lineTo(-8,10); ctx.closePath(); ctx.fill();
...
async function act(action) {
    ...
    try {
        ...
        if (r.status==='win') {
            log('VICTOIRE !','ok'); document.getElementById('s-status').textContent='WIN';
            ...
            log('Défaite ('+r.status+')','err'); document.getElementById('s-status').textContent='LOSE';
            setControls(false); stopAuto(); sessionId=null;
        }
...
function autoAction(s) {
    ...
    if (t==='ttt') {
        const b=s.board;
        for (let r=0;r<3;r++) for (let c=0;c<3;c++) if (!b[r][c]) return ''+r+c;
        return '00';
    }
    if (t==='car') {
    ...
    if (t==='traffic') {
        ...
        if (ahead<8) {
            const alt=gaps.filter(g=>g.lane!==lane&&(g.ahead??0)>12);
            if (alt.length) return alt[0].lane<lane?'left':'right';
            return 'brake';
        }
    ...
    if (t==='lander') {
        const obs=s.observation??[], alt=obs[0]??10, vx=obs[1]??0, vy=obs[2]??0, dx=obs[3]??0;
...
function bfsAction(s) {
    if (!s?.grid) return 'up';
    for (let y=0;y<s.grid.length;y++) for (let x=0;x<s.grid[y].length;x++) {
        const ch=s.grid[y][x]; if (ch&&ch!=='?') memMap[x+','+y]=ch;
    }
...
function toggleAuto() {
    ...
    autoTimer=setInterval(async()=>{ if(!curState||!sessionId){stopAuto();return;} await act(autoAction(curState)); }, speed);
}
function stopAuto() {
...
function setControls(on) {
    ['btn-stop','btn-auto','k-up','k-down','k-left','k-right'].forEach(id=>document.getElementById(id).disabled=!on);
}
